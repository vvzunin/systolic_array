...
(   
    ...
    always_comb begin
        ...
        case (state)
            ...
            LOAD_WEIGHTS:
                if (parser_valid & (parser_cmd == `CMD_FETCH_DATA   )) next_state = FETCH_DATA;
            FETCH_DATA:
            ...
            SEND_BACK:
                if (                                      sb_finished) next_state = IDLE;     
    ...
    always_comb begin
        ...
        case (sb_state)
            IDLE_SB:
                if(next_state == SEND_BACK) sb_next_state = SEND_BYTE_SB;
            SEND_BYTE_SB:
                sb_next_state = SET_TX_VALID_LOW;
            SET_TX_VALID_LOW:
    ...
    assign sa_weights_load = (state == LOAD_WEIGHTS) & (next_state == FETCH_DATA);
    ...
    always_ff @(posedge clk) begin
        case (state)
            IDLE: begin
                tx_valid <= '0;
                if (parser_valid & (parser_cmd == `CMD_FETCH_WEIGHTS)) begin
                    row_ptr <= '0;
                    col_ptr <= '0;
                end
            end
            FETCH_WEIGHTS:
                if (parser_valid & (parser_cmd == `CMD_DATA)) begin
            ...
            SEND_BACK: begin
                ...
                SEND_BYTE_SB: begin
                    ...
                    if ((col_ptr == ARRAY_W_L - 1) & (bit_ptr == DATA_WIDTH - 8)) begin
                        row_ptr <= row_ptr + 'd1;
                        col_ptr <= '0;
                    end
                    else if (bit_ptr == DATA_WIDTH - 8)
                        col_ptr <= col_ptr + 'd1;
                end
                SET_TX_VALID_LOW: begin
                    tx_valid <= '0;
                end
                endcase
            end
    ...
    always_ff @( posedge clk ) begin
        if (!rstn) begin
            ...
            sb_finished <= (row_ptr == ARRAY_A_W - 1) & (col_ptr == ARRAY_W_L - 1) & (bit_ptr == DATA_WIDTH - 8)
                            & (sb_state == WAIT_SB) & (sb_next_state == SEND_BYTE_SB);
        end
    end
